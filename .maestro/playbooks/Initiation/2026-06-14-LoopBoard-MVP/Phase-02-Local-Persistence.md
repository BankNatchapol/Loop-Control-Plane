...
- [x] Test persistence and context generation:
  ...
  - Completion note: Strengthened `tests/persistence-integration.test.ts` to verify that a migrated and seeded SQLite database preserves moved task state and events across database reopen, that generated `task.md`/`context.md`/`handoff.md`/`events.jsonl` artifacts are created, and that a later persisted task event refreshes an existing `events.jsonl` through the same context sync helper used by mutation routes. Verified the full persistence/context path with `npm run lint`, `npm run typecheck`, `npm test`, `npm run db:migrate`, `npm run db:seed`, and `npm run contexts:generate`. Confirmed the seeded board appears through `GET /api/board` with 1 project, 3 features, and 7 tasks before and after a Next dev-server restart. No task images were associated with this checkbox, so analyzed 0 images. This folder is not a Git repository, so no commit or push was possible.
